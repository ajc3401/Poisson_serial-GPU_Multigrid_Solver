...
#include <math.h>
//template<class T>
//Multigrid<T>::Multigrid(CRS_Matrix<T>& _L, CRS_Matrix<T>& _U, CRS_Matrix<T>& _Dinv, Vector<T>& _v, Vector<T>& _f, const size_t n_GS2, const size_t n_Jac, const size_t n_outer, const size_t n_coarsen) :
//	m_L{_L},
//	m_U{_U},
//	m_Dinv{_Dinv},
//	m_v{_v},
...

{
	...
	for (size_t coarsenIdx = 0; coarsenIdx < _ncoarsen; coarsenIdx++)
	{
		...
		if (coarsenIdx > 0)
			std::cout << "coarsen is " << coarsenIdx << std::endl;
